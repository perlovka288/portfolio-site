...
{
    $stmt = $pdo->prepare("
        INSERT INTO pack_resources (type, title, description, preview_image, telegram_url, file_url, video_url)
        VALUES (:type, :title, :description, :preview_image, :telegram_url, :file_url, :video_url)
    ");
    $stmt->execute([
        ':type'          => $data['type'] ?? '',
        ':title'         => $data['title'] ?? '',
