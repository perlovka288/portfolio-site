...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $pay_screenshot = '';
    $example_img    = '';
    $target_dir     = __DIR__ . '/uploads/orders/';
    ...
    try {
        // PostgreSQL: используем RETURNING id вместо lastInsertId()
        $stmt = $pdo->prepare("
            INSERT INTO orders (username, telegram, service_key, details, screenshot, example_photo, status, created_at)
            VALUES (?, ?, ?, ?, ?, ?, 'pending', NOW())
            RETURNING id
        ");
        ...
        if (!empty($my_chat_id)) {
            ...
            if (!empty($pay_screenshot) && file_exists($target_dir . $pay_screenshot)) {
                $media[] = ['type' => 'photo', 'media' => $host_url . 'uploads/orders/' . $pay_screenshot];
            }
            if (!empty($example_img) && file_exists($target_dir . $example_img)) {
                $media[] = ['type' => 'photo', 'media' => $host_url . 'uploads/orders/' . $example_img];
            }
...
<?php if (!empty($success_msg)): ?>
    <div style="background:rgba(0,255,163,.1);border:1px solid #00ffa3;color:#00ffa3;padding:15px;border-radius:8px;text-align:center;margin-bottom:20px;font-weight:bold;">
        <?= $success_msg ?>
    </div>
<?php endif; ?>

<?php if (!empty($error_msg)): ?>
    <div style="background:rgba(239,68,68,.1);border:1px solid #ef4444;color:#ef4444;padding:15px;border-radius:8px;text-align:center;margin-bottom:20px;">
        <?= $error_msg ?>
    </div>
<?php endif; ?>
...
<?php if (!$rules_accepted && $_SERVER['REQUEST_METHOD'] !== 'POST'): ?>
    <div class="rules-card" style="background:#111116;border:1px solid #2a2a38;padding:30px;border-radius:16px;box-shadow:0 18px 45px rgba(0,0,0,.28);">
        ...
        <ul style="display:grid;gap:12px;color:#e8e8f0;padding-left:20px;line-height:1.55;margin:0 0 24px;">
            <li>Заказ выполняется в течение 5 дней. При предоплате 50% — вне очереди.</li>
            <li>Деньги не возвращаются.</li>
            <li>Статус заказа в боте: <a href="<?= htmlspecialchars($bot_link) ?>" target="_blank" style="color:#c76860;">@kostlimdznbot</a>.</li>
            <li>По личным вопросам: <a href="<?= htmlspecialchars($support_tg) ?>" target="_blank" style="color:#c76860;">@Perlo_ovka</a>.</li>
        </ul>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
            <a href="<?= htmlspecialchars($accept_url) ?>" style="background:#a95851;color:#fff;text-align:center;text-decoration:none;padding:14px 16px;border-radius:8px;font-weight:900;text-transform:uppercase;">Согласиться</a>
...
<?php else: ?>
    <div style="background:#111116;border:1px solid #1f1f2a;padding:30px;border-radius:16px;">
        <div style="text-align:center;margin-bottom:25px;">
            <a href="index.php" style="color:#a95851;text-decoration:none;font-size:13px;">← На главную к портфолио</a>
            <h2 style="color:#fff;margin-top:10px;text-transform:uppercase;letter-spacing:1px;">📋 Заполнить ТЗ</h2>
        </div>
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <div style="margin-bottom:16px;">
                <label style="color:#8a8a93;font-size:12px;font-weight:bold;text-transform:uppercase;">Telegram @username</label>
            ...
            <div style="margin-bottom:16px;">
                ...
                <select name="service" style="background:#16161f;border:1px solid #262633;color:#fff;padding:12px;border-radius:8px;width:100%;box-sizing:border-box;margin-top:6px;">
                    <?php foreach ($services as $s): ?>
                        <option value="<?= $s['category_key'] ?>" <?= ($selected_service === $s['category_key']) ? 'selected' : '' ?>>
            ...
            <div style="margin-bottom:16px;">
                <label style="color:#8a8a93;font-size:12px;font-weight:bold;text-transform:uppercase;">Детали заказа (ТЗ)</label>
                <textarea name="details" required placeholder="Опиши цвета, текст, стиль..." style="background:#16161f;border:1px solid #262633;color:#fff;padding:12px;border-radius:8px;width:100%;box-sizing:border-box;height:100px;resize:none;margin-top:6px;"></textarea>
            </div>
            <div style="margin-bottom:16px;border:1px dashed #262633;padding:12px;border-radius:8px;">
                <label style="color:#fff;font-size:12px;font-weight:bold;">📸 Скриншот оплаты:</label>
            ...
            <div style="margin-bottom:24px;border:1px dashed #262633;padding:12px;border-radius:8px;">
                <label style="color:#fff;font-size:12px;font-weight:bold;">🖼️ Референс / пример:</label>
                <input type="file" name="example_photo" accept="image/*" style="margin-top:5px;color:#8a8a93;font-size:12px;">
            </div>
            <button type="submit" style="background:#a95851;color:#fff;border:none;padding:15px;border-radius:8px;width:100%;font-weight:bold;cursor:pointer;text-transform:uppercase;font-size:14px;">
