...
// ===================== APPEALS =====================
// Закрыть обращение (только новое — клиент должен создать новое)
                    ...
                    <div class="panel" data-panel="appeals">
                        ...
                        <?php foreach ($appeals as $ap): ?>
                            ...
                            <div style="border-radius:12px;padding:16px 18px;background:<?= $isOpen ? 'rgba(249,115,22,.06)' : ($ap['status'] === 'closed' ? 'rgba(239,68,68,.04)' : '#111116') ?>;border:1px solid <?= $isOpen ? 'rgba(249,115,22,.35)' : ($ap['status'] === 'closed' ? 'rgba(239,68,68,.2)' : '#20202c') ?>;">
                                ...
                                <?php endif; ?>
                                <form action="" method="POST" style="display:grid;gap:8px;<?= $ap['status'] === 'closed' ? 'opacity:.4;pointer-events:none;' : '' ?>">
                                    ...
                                    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                                        ...
                                        <button type="submit" name="close_appeal" onclick="return confirm('Закрыть обращение? Клиент не сможет дописать в него — только создать новое.')" style="border:1px solid rgba(239,68,68,.4);border-radius:9px;padding:10px 18px;background:rgba(239,68,68,.1);color:#f87171;font-weight:800;cursor:pointer;font-family:Montserrat,sans-serif;font-size:13px;">🔒 Закрыть обращение</button>
                                    </div>
                                </form>
                            </div>
                        <?php endforeach; ?>
