 ...
 * Полностью автономная гибридная сборка (PHP + JS-перехватчик).
 */
...
// Жесткий системный промпт для контроля модели
...
// ── СЦЕНАРИЙ 2: Прямой POST запрос от старого JS-кода (на всякий случай) ──
...
// ── СЦЕНАРИЙ 3: index.php загружает этот файл через тег <script> ──────────
...
(function() {
    ...
    // Подгружаем токен и промпт в фоне сразу при загрузке страницы
    originalFetch('/ai_support.php?get_internal_config_raw=1')
        .then(res => res.json())
        .then(data => { if (data.ok) geminiConfig = data; })
        .catch(err => console.error('Ошибка инициализации ИИ:', err));

    // Перехватываем стандартный fetch чата на лету в браузере
        ...
        if (typeof url === 'string' && url.includes('ai_support.php') && options && options.method === 'POST') {
            try {
                ...
                if (!geminiConfig || !geminiConfig.key) {
                    return new Response(JSON.stringify({ ok: false, reply: 'Соединение с ИИ настраивается, повторите отправку через секунду.' }), { status: 200 });
                }
                ...
                // Запрос летит напрямую из браузера на стабильный эндпоинт Google API v1, минуя прокси Render!
                ...
                if (geminiData.error) {
                    return new Response(JSON.stringify({ ok: false, reply: 'Не удалось подключиться к ИИ. Попробуйте обновить страницу.' }), { status: 200 });
                }
                ...
                // Отдаем твоему фронтенд-скрипту ответ в привычном ему формате
