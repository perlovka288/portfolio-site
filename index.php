...
// Safari блокирует куки без этих параметров при переходах из других приложений (TG)
...
} catch (Throwable $daErr) {
    // Если donationalerts недоступен — работаем без него
    if (!function_exists('daEnsureAccessToken')) {
        function daEnsureAccessToken($pdo) { return null; }
        function daGetDonations($pdo, $limit=200) { return []; }
        function daGetCurrentMonthDonationTotalUsd($donations) { return 0.0; }
        function daGetCurrentMonthPayoutStats($pdo) { return ['gross'=>0,'count'=>0,'commission'=>0,'net'=>0]; }
        function daExchangeAuthorizationCode($pdo, $code) { return false; }
    }
}
...
$tgProfile = [];
$sid = session_id();
$likeUserKey = !empty($_SESSION['tg_chat_id']) ? ('tg:' . (string)$_SESSION['tg_chat_id']) : ('sid:' . $sid);
...
if (isset($_POST['like_work'])) {
    ...
    if ($workId > 0) {
        try {
            ...
            $currentLikeKey = !empty($_SESSION['tg_chat_id']) ? ('tg:' . (string)$_SESSION['tg_chat_id']) : ('sid:' . session_id());
            $chk = $pdo->prepare("SELECT id FROM portfolio_likes WHERE portfolio_id = ? AND session_id = ?");
            $chk->execute([$workId, $currentLikeKey]);
            $existing = $chk->fetch();
            if ($existing) {
                // Анлайк
                $pdo->prepare("DELETE FROM portfolio_likes WHERE portfolio_id = ? AND session_id = ?")->execute([$workId, $currentLikeKey]);
                $liked = false;
            } else {
                // Лайк — ON CONFLICT DO NOTHING страхует от гонки
                $pdo->prepare("INSERT INTO portfolio_likes (portfolio_id, session_id, ip) VALUES (?, ?, ?) ON CONFLICT (portfolio_id, session_id) DO NOTHING")->execute([$workId, $currentLikeKey, $ip]);
                $liked = true;
            }
...
function imgSrc(string $val, string $base = 'uploads/'): string {
    ...
    // Используем абсолютный путь от корня — работает при любом URL
...
/* Появление карточек портфолио при скролле — "выходят из темноты", а не
   ...
   снизу + размытие, которое снимается по мере проявления. */
.scroll-reveal {
    opacity: 0;
...
<div class="tg-modal-overlay" id="tgModalOverlay">
  <div class="tg-modal" id="tgModal">
    ...
    <a href="https://t.me/kostlimdznbot?start=link_<?= htmlspecialchars($linkCode ?? '') ?>"
       class="tg-bot-open-btn" id="openBotBtn"
       onclick="openTgBotLink(event, this.href)">
...
// ── Открыть бота TG (фикс для iOS Safari — target=_blank там глючит) ──
function openTgBotLink(e, url) {
    e.preventDefault();
    startPolling();
    // На iOS Safari используем location.href чтобы Telegram открылся корректно
...
// ── Появление карточек портфолио "из темноты" при скролле ──
(function() {
    var revealItems = document.querySelectorAll('.scroll-reveal');
    if (!revealItems.length) return;
    if (!('IntersectionObserver' in window)) {
        // Старые браузеры без поддержки — просто показываем всё сразу.
    ...
    var revealObserver = new IntersectionObserver(function(entries, obs) {
        entries.forEach(function(entry) {
            if (entry.isIntersecting) {
                // Небольшая последовательная задержка по позиции в сетке —
                // карточки "проступают" не все одновременно, а волной.
                var delay = (Array.prototype.indexOf.call(revealItems, entry.target) % 3) * 90;
...
(function(){
    try {
        ...
        tg.ready();
        // Отправляем ПОДПИСАННУЮ строку initData скрыто, телом POST-запроса —
        // в адресной строке браузера (в т.ч. в Chrome) ничего не отображается,
        // и подпись проверяется на сервере токеном бота (подделать нельзя).
