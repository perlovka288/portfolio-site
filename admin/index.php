...
function publishPortfolioToChannel(PDO $pdo, string $uploadDir, array $case): bool
{
    ...
    // Если в БД полный URL (imgbb или любой https) — скачиваем во temp
...
// ── ImgBB upload с ротацией до 3 ключей ─────────────────────
// Render — ephemeral сервер: файлы на диске уничтожаются при каждом деплое.
// Поэтому ВСЕ изображения загружаем на imgbb и храним полный https:// URL в БД.
//
// Задай в Render Environment Variables:
//   IMGBB_API_KEY   — первый ключ (обязательно)
//   IMGBB_API_KEY2  — второй ключ (опционально)
//   IMGBB_API_KEY3  — третий ключ (опционально)
// Если первый ушёл в кулдаун (лимит) — автоматически берётся следующий.
function uploadToImgBB(string $tmpPath, string $name = 'image'): string
{
    if (!is_file($tmpPath)) {
        error_log("ImgBB: file not found ($tmpPath)");
        return '';
    }
    ...
    if (empty($keys)) {
        error_log("ImgBB: no API keys set (IMGBB_API_KEY, IMGBB_API_KEY2, IMGBB_API_KEY3)");
        return '';
    }
    ...
    foreach ($keys as $index => $apiKey) {
        ...
        if ($res === false || $res === '') {
            error_log("ImgBB key#" . ($index + 1) . " curl error: $cerr");
        ...
        if ($url !== '') {
            error_log("ImgBB key#" . ($index + 1) . " OK => $url");
            return $url;
        }

        // Проверяем код ошибки — 429 или "Too Many Requests" = лимит, берём следующий ключ
        ...
        if ($statusCode === 429 || stripos($errMsg, 'too many') !== false || stripos($errMsg, 'limit') !== false) {
            continue; // лимит — пробуем следующий ключ
        }

        // Другая ошибка (неверный ключ, неверный формат) — дальше не имеет смысла с этим ключом
        // но всё равно пробуем следующий
        continue;
    }
...
// ── Загрузка изображения: ТОЛЬКО imgbb (диск на Render ephemeral) ──
function uploadImage(string $field, string $prefix, string $uploadDir): string
{
    ...
    if ($err === UPLOAD_ERR_INI_SIZE || $err === UPLOAD_ERR_FORM_SIZE) {
        $message = '❌ Файл слишком большой. Уменьши размер до 8 МБ.';
        return '';
    }
    ...
    // Грузим на imgbb — URL переживёт любой деплой
    ...
    // Fallback: локальный диск (работает только до следующего деплоя!)
    ...
    if (is_writable($uploadDir)) {
        ...
        if (move_uploaded_file($tmp, $dest)) {
            error_log("UPLOAD[$field]: local fallback OK => $filename");
            return $filename; // возвращаем только имя — imgSrc() добавит uploads/
        }
    }

    error_log("UPLOAD[$field]: both methods failed");
    $message = '❌ Не удалось загрузить изображение. Проверь IMGBB_API_KEY в переменных окружения Render.';
    return '';
}
...
// ── Resolve image src: полный URL или uploads/ + имя файла ──
...
if (isset($_POST['upload_site_avatar'])) {
    ...
    if ($newAvatar !== '') {
        ...
        if ($message === '') $message = '❌ Не удалось загрузить аватарку. Проверь формат (jpg, png, webp, gif) и IMGBB_API_KEY.';
    }
}
...
if (isset($_POST['add_portfolio'])) {
    ...
    if ($title === '') {
        ...
        if ($message === '') $message = '❌ Загрузи главное изображение. Убедись что IMGBB_API_KEY задан в Render Environment Variables.';
    } else {
        $stmt = $pdo->prepare("
        ...
        $message = '✅ Кейс добавлен в портфолио.' . ($postedToChannel ? ' Пост отправлен в Telegram-канал.' : ' Но пост в Telegram-канал не отправился: ' . ($telegramLastError !== '' ? $telegramLastError : 'проверь настройки бота.'));
    }
}

if (isset($_GET['delete_portfolio_id'])) {
    $del_id = (int)$_GET['delete_portfolio_id'];
    // Удалять локальные файлы не нужно (imgbb хранит сам), но на всякий случай чистим если есть
...
if (isset($_POST['update_portfolio_media'])) {
    ...
    if ($currentFiles) {
        ...
        if ($newMain !== '') {
            $pdo->prepare("UPDATE portfolio SET image = ? WHERE id = ?")->execute([$newMain, $caseId]);
        }
        if ($newAvatar !== '') {
            $pdo->prepare("UPDATE portfolio SET avatar_image = ? WHERE id = ?")->execute([$newAvatar, $caseId]);
        }
...
if (isset($_GET['delete_price_id'])) {
    ...
    $pdo->prepare("DELETE FROM prices WHERE id = ?")->execute([$del_id]);
...
if (isset($_GET['delete_portfolio_category_id'])) {
    ...
    $pdo->prepare("DELETE FROM portfolio_categories WHERE id = ?")->execute([$catId]);
...
<head>
    ...
    <style>
        * { scrollbar-width: thin; scrollbar-color: #f97316 #111116; }
        ...
        .btn-panel { width: 100%; margin-top: 14px; border: none; border-radius: 10px; padding: 13px 16px; background: linear-gradient(135deg, #fb923c, #f97316); color: #fff; font-weight: 900; cursor: pointer; text-transform: uppercase; font-family: Montserrat, sans-serif; letter-spacing: 1px; font-size: 13px; box-shadow: 0 8px 24px rgba(249,115,22,.30); transition: .2s; }
    ...
    <div class="admin-top">
        <div class="admin-title">
            ...
            <div class="admin-meta">
                ...
                <?php else: ?>
                    <span class="warn">⚠️ IMGBB_API_KEY не задан! Фото не переживут деплой.</span>
                <?php endif; ?>
            </div>
        </div>
        ...
        <aside>
            ...
            <section class="panel" data-panel="avatar">
                ...
                <div class="avatar-preview-wrap">
                    ...
                    <img
                        src="<?= htmlspecialchars($avatarSrc ?: 'https://i.imgur.com/w9NThbA.png') ?>"
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="avatar-hint">Форматы: jpg, png, webp, gif. Рекомендуется квадратное фото (512×512). После загрузки аватарка сохраняется на imgbb и не потеряется при деплое.</div>
